(function(){
  ...
  let LANG = new URLSearchParams(location.search).get('lang')?.toLowerCase()||'en';
  ...
  function openModal(key){
    ...
    $('#modalContent').innerHTML=`
      ...
      ${links.length?`<div><strong>${LANG==='ru'?'Ссылки':'Links'}:</strong>${links.map(l=>`<a class="link" href="${l.url}" target="_blank">${normalizeText(l.label||l)}</a>`).join(' ')}</div>`:''}`;
  ...
  async function boot(){
    ...
    const real=(async()=>{
      ...
      const [_,__,data]=await Promise.all([
        ...
        fetch(LANG==='ru'?'projects_ru.json':'projects_en.json',{cache:'no-store'}).then(r=>r.json())
      ]);
      PROJECTS_DATA=data;
    })();
